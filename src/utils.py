...
def kernel(t_x, s_x, kernel):
    """
    Apply kernel function and return the distance (MustaD)

    param t_x: feature vector
    param s_x: feature vector
    return: kernel based distance between two input vectors
    """
    # batch matrix product input (b x n x m), mat2 (b x m x p) => output (b x n x p)
...
def normalize(mx):
    ...
    """
    rowsum = np.array(mx.sum(1)) # row 기준으로 모든 요소 sum
    rowsum = (rowsum==0)*1+rowsum # 합이 0이면 1로, 아니면 그대로
    r_inv = np.power(rowsum, -1).flatten() # inverse 형태로 만들고 flatten
    r_inv[np.isinf(r_inv)] = 0 # infinite면 0으로 치환
    r_mat_inv = sp.diags(r_inv) # sparse matrix로 만듦
    mx = r_mat_inv.dot(mx) 
    return mx

# adjacency matrix -> sparse matrix shape -> row-normalize
...
# adapted from PetarV/GAT => 이 부분 왜 있지? 궁그미
