...
def choose_model(conf):
    if conf['model_name'] == 'GCN':
        ...
        model = GraphSAGE_student(in_feats=features.shape[1],
...
if __name__ == '__main__':
    ...
    # tensorboard name
    board_name = "MustaD_student_"+str(conf['model_name'])+"dataset_"+str(conf['dataset'])+"lbd_pred:"+str(conf['lbd_pred'])+"lbd_embd"+str(conf['lbd_embd'])
    ...
    for epoch in range(500):
        loss_train, acc_train, sel_train, masks = train(masks, conf, teacher_conf)
        loss_val, acc_val, sel_val, masks = validate(masks, conf, teacher_conf)
        if (epoch + 1) % 10 == 0:
